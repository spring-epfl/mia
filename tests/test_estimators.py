...
@pytest.mark.parametrize('shadow_model_fn,model_serializer', [
...
def test_shadow_models_are_created_and_data_is_transformed(
    ...
    X_shadow, y_shadow = smb.fit_transform(
            X_train, y_train,
            fit_kwargs=dict(epochs=5, verbose=False))

    # X_shadow are prediction vectors for random examples in X_train
    assert len(X_shadow) == len(y_shadow) == (
            2 * num_models * SHADOW_DATASET_SIZE)
...
def test_shadow_transform_with_custom_shadow_indices(
        data, shadow_model_fn, model_serializer,
        num_models, max_models_for_transform):
...
@pytest.mark.parametrize(
    ...
    'attack_model_fn,attack_model_serializer', [
        (keras_shadow_model_fn, None,
         keras_attack_model_fn, None),
        (keras_shadow_model_fn, None,
         torch_attack_model_fn, None),
        (torch_shadow_model_fn, None,
         torch_attack_model_fn, None),
        (torch_shadow_model_fn, None,
         keras_attack_model_fn, None),
...
def test_attack_models_are_created(
        data, shadow_model_fn, shadow_model_serializer,
        attack_model_fn, attack_model_serializer):
